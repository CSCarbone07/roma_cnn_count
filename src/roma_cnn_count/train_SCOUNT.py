...
if __name__ == "__main__":
    ...
    train_set = FruitCounting(root=dataset_root,
                              set='train')#, countClasses = conf.countClasses)
    test_set = FruitCounting(root=dataset_root,
                             set='test')#, countClasses = conf.countClasses)
    save_path = save_path
    ...
    engine = SCOUNT_Engine(model=model, train_set=train_set, validation_set=test_set, test_set=test_set, seed=1, batch_size=5, save_path=save_path, log_path=log_path, num_epochs=conf.epochs, countClasses = conf.countClasses, hotEncoded = conf.hotEncoded)
